...
> **מטרה:** שכתוב מלא של site_generator.py — CLAUDE.md תובעני שמשתמש ב-maori-ui.
...
### 2. שימוש בספרייה (חובה)
```
@tottemai/maori-ui IS INSTALLED — NEVER reimplement its components.

Import:
  import { TextReveal, ScrollReveal, MagneticButton } from "@tottemai/maori-ui"
  import { PageTransition, CustomCursor, ScrollProgress } from "@tottemai/maori-ui"
  import { Parallax, Marquee, CountUp, Reveal3D } from "@tottemai/maori-ui"
  import { Button, Card, Section, Container, SectionTitle } from "@tottemai/maori-ui"
  import { Navbar, Footer, MobileMenu } from "@tottemai/maori-ui"
...
```
1.  package.json              ← maori-ui + framer-motion + gsap + lenis
2.  next.config.mjs           ← transpilePackages: ["@tottemai/maori-ui"]
3.  tsconfig.json
...
- [ ] בדוק `cat package.json | grep maori-ui` → קיים
- [ ] בדוק `grep -r "TextReveal" components/` → מופיע
- [ ] בדוק `grep -r "from \"@tottemai/maori-ui\"" components/` → מופיע
